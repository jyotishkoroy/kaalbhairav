  ...
  const service = {
    ...
    from: vi.fn((table: string) => {
      ...
      if (table === 'birth_profiles') {
        chain.select = vi.fn(() => ({
          eq: vi.fn(() => Promise.resolve({ data: [{ id: 'profile-1' }], error: null })),
        }))
      }
      if (table === 'chart_calculations') {
        chain.select = vi.fn(() => ({
          eq: vi.fn(() => Promise.resolve({ data: [{ id: 'calc-1' }], error: null })),
        }))
      }
      if (table === 'chart_json_versions') {
        chain.select = vi.fn(() => ({
          eq: vi.fn(() => Promise.resolve({ data: [{ id: 'chart-1' }], error: null })),
        }))
      }
      if (table === 'astro_chat_sessions') {
        chain.select = vi.fn(() => ({
          eq: vi.fn(() => Promise.resolve({ data: [{ id: 'session-1' }], error: null })),
        }))
      }
      return chain
    }),
...
describe('deleteAccountAndUserData', () => {
  ...
  it('does not include private data in deleted_users payload', async () => {
    ...
    expect(JSON.stringify(payload)).not.toMatch(/email|birth|chart|conversation|user_id/i)
  })
