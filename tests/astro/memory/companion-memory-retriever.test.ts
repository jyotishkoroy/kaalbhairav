...
describe("companion memory retriever", () => {
  ...
  it("feature flag false preserves previous path", async () => expect((await retrieveCompanionMemorySafely({ userId: "u1", store: makeStore([]), env: { ASTRO_COMPANION_MEMORY_ENABLED: "true", ASTRO_COMPANION_MEMORY_RETRIEVE_ENABLED: "true", ASTRO_MEMORY_RELEVANCE_GATE_ENABLED: "false" } })).source).toBe("supabase"));
  it("feature flag true applies relevance gate", async () => expect((await retrieveCompanionMemorySafely({ userId: "u1", topic: "relationship", question: "partner issue", store: makeStore([{ id: "1", user_id: "u1", memory_type: "preference", topic: "career", content: "Career guidance about work", confidence: "high", last_seen_at: "2026-04-01T00:00:00.000Z", created_at: "2026-04-01T00:00:00.000Z", updated_at: "2026-04-01T00:00:00.000Z" }]), env: { ...env, ASTRO_MEMORY_RELEVANCE_GATE_ENABLED: "true" } })).used).toBe(false));
  ...
  it("career memory summary remains natural", async () => expect((await retrieveCompanionMemorySafely({ userId: "u1", store: makeStore([{ id: "1", user_id: "u1", memory_type: "preference", topic: "career", content: "Previous concern: career guidance.", confidence: "high", last_seen_at: "2026-04-01T00:00:00.000Z", created_at: "2026-04-01T00:00:00.000Z", updated_at: "2026-04-01T00:00:00.000Z" }]), env: { ...env, ASTRO_MEMORY_RELEVANCE_GATE_ENABLED: "true" } })).summary ?? "").not.toMatch(/Previous concern:/i));
  it("money prompt without job wording blocks career memory", async () => expect((await retrieveCompanionMemorySafely({ userId: "u1", topic: "money", question: "budget only", store: makeStore([{ id: "1", user_id: "u1", memory_type: "preference", topic: "career", content: "Career guidance about work", confidence: "high", last_seen_at: "2026-04-01T00:00:00.000Z", created_at: "2026-04-01T00:00:00.000Z", updatedAt: "2026-04-01T00:00:00.000Z" } as never]), env: { ...env, ASTRO_MEMORY_RELEVANCE_GATE_ENABLED: "true" } })).used).toBe(false));
});
