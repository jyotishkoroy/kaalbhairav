...
vi.mock('@/lib/astro/chart-json-persistence', () => ({
  ...
  persistCanonicalChartJsonV2: vi.fn(async () => ({ chartVersionId: 'cv-current', chartVersion: 11, chartJson: { metadata: { chartVersionId: 'cv-current', chartVersion: 11 } } })),
}))
...
vi.mock('@/lib/astro/current-chart-version', () => ({
  loadCurrentAstroChartForUser: vi.fn(async () => ({
    ok: true,
    profile: { id: '11111111-1111-4111-8111-111111111111', user_id: 'u1', status: 'active', current_chart_version_id: 'cv-current' },
    chartVersion: {
      id: 'cv-current',
...
function makeCanonicalChartJsonV2({
  ...
  return {
    ...
    metadata: {
      ...
      ...(typeof chartVersion === 'number' ? { chartVersion } : {}),
      inputHash: 'input-hash',
      settingsHash: 'settings-hash',
      engineVersion: 'engine-1',
      ephemerisVersion: 'ephemeris-1',
    ...
    sections: {
      ...
      houses: computedSection({ placements: { Sun: 10, Moon: 11 } }),
  ...
  it('calls the atomic RPC and does not directly insert chart rows on success', async () => {
    ...
    vi.mocked(createServiceClient).mockReturnValue({
      ...
      from: vi.fn((table: string) => {
        if (table === 'birth_profiles') return makeQuery({ id: profileId, user_id: user.id, encrypted_birth_data: 'x', status: 'active', current_chart_version_id: 'cv-current' })
        ...
        if (table === 'chart_json_versions') return { insert: chartInsertSpy, select: () => makeQuery({ id: 'cv-current', profile_id: profileId, user_id: user.id, chart_json: makeCanonicalChartJsonV2({ profileId, chartVersionId: 'cv-current', chartVersion: 11 }), chart_version: 11, status: 'completed', is_current: true }) }
