...
describe("structured intent router", () => {
  ...
  it("routes job business study choice to career with secondary intents", () => expect(route("Should I focus on job, business, or study?")).toMatchObject({ primaryIntent: "career", secondaryIntents: ["job", "business", "study"] }));
  ...
  it("routes education versus work to education", () => expect(route("Should I continue education or start working?")).toMatchObject({ primaryIntent: "education", secondaryIntents: ["career"] }));
  ...
  it("routes go abroad to foreign settlement", () => expect(route("Will I go abroad?")).toMatchObject({ primaryIntent: "foreign_settlement" }));
  it("routes foreign settlement guaranteed to safety", () => expect(route("Is foreign settlement guaranteed?")).toMatchObject({ primaryIntent: "foreign_settlement", mode: "safety" }));
  it("routes bad sleep remedy to sleep", () => expect(route("Give me remedy for bad sleep.")).toMatchObject({ primaryIntent: "sleep", mode: "remedy" }));
