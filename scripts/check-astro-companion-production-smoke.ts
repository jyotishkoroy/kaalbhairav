...
function parseArgs(argv: string[]) {
  const args = { baseUrl: process.env.ASTRO_COMPANION_PRODUCTION_BASE_URL || process.env.ASTRO_COMPANION_LIVE_BASE_URL || "https://tarayai.com", outputDir: path.join(process.cwd(), "artifacts"), timeoutMs: 30000 };
...
async function run() {
  ...
  results.push({ id: "lagna_exact", passed: page.status !== 404, failures: page.status === 404 ? ["route_missing:/astro/v2"] : [], warnings: [], live: page });
  for (const prompt of getCompanionSmokePrompts()) {
    const live = await fetchEndpoint(baseUrl, "/api/astro/v2/reading", args.timeoutMs, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(buildAstroReadingPayload(prompt)) });
    ...
    const warnings = (live.status === 401 || live.status === 403 || /auth|profile|context/i.test(live.answer)) ? ["route_available_but_auth_required"] : evaluation.warnings;
