...
const INTERNAL_PROBE_PATTERNS = [
  /\breveal\b[\s\S]{0,20}\bprompt\b/i,
  /\bsystem prompt\b/i,
  /\bdeveloper prompt\b/i,
  /\bignore previous instructions\b/i,
  /\bbypass safety\b/i,
  /\bleak\b[\s\S]{0,20}\bdatabase\b/i,
  /\bdump\b[\s\S]{0,20}\bdatabase\b/i,
  /\bsteal\b[\s\S]{0,20}\bsession cookie\b/i,
  /\bsession cookie\b/i,
  /\bservice role key\b/i,
  ...
  /\bauth token\b/i,
  /\boauth code\b/i,
  /\b(ai\s*model|model|server|database rows|logs|credentials|token|secret|groq|ollama|openai|llm)\b/i,
];
...
const LANGUAGE_BLOCK_PATTERNS = [/[\u0900-\u097F]/, /[\u0980-\u09FF]/, /[\u0600-\u06FF]/, /\bkya\b/i, /\bkaise\b/i, /\bmera\b/i, /\bmeri\b/i, /\bmujhe\b/i, /\bshaadi\b/i, /\bpaisa\b/i, /\bamar\b/i, /\bamr\b/i, /\bbiye\b/i, /\bkobe\b/i, /\bhobe\b/i, /\bbhalo\b/i, /\btomar\b/i, /\bami\b/i, /\btumi\b/i, /\bkorbo\b/i];
...
function hasMeaningfulAsciiLetter(s: string): boolean { return /[a-z]/i.test(s); }
...
export function guardOneShotAstroQuestion(question: string): AstroQuestionGuardResult {
  ...
  if (!hasMeaningfulAsciiLetter(normalized) && normalized.length < 2) return { allowed: false, code: "empty_question", answer: "aadesh: Please enter a question to receive guidance." };
  if (!hasMeaningfulAsciiLetter(normalized)) return { allowed: false, code: "malformed_question", answer: "aadesh: Please enter a clear English question to receive guidance." };
