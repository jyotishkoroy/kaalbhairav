...
import { classifyVedicTopic, type VedicTopic } from "../rag/vedic-topic-classifier.ts";
...
export async function answerCanonicalAstroQuestion(input: {
  ...
  if (/(death|lifespan|suicide|self-harm)/i.test(input.question)) {
    return { answer: "aadesh: I cannot help predict an exact death date or lifespan. If this is about safety or distress, please contact local emergency services or a trusted person right now." };
  }
  ...
  const answer = buildVedicStyleAnswer({
    question: input.question,
    topic,
    facts: chartContext.normalizedFacts,
    safetyMode: topic.startsWith("safety") || topic === "security" ? "safety" : "normal",
  });
