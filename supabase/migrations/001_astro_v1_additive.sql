...
create trigger trg_birth_profiles_updated before update on public.birth_profiles
...
create index if not exists idx_astrology_settings_user_id on public.astrology_settings(user_id);
drop trigger if exists trg_astrology_settings_updated on public.astrology_settings;
create trigger trg_astrology_settings_updated before update on public.astrology_settings
  for each row execute function public.set_updated_at();
...
create policy astrology_settings_owner_select on public.astrology_settings
  for select using (auth.uid() = user_id);
...
create index if not exists idx_chart_calculations_user_id on public.chart_calculations(user_id);
create index if not exists idx_chart_calculations_profile_status on public.chart_calculations(profile_id, status);
...
create policy chart_calculations_owner_select on public.chart_calculations
  for select using (auth.uid() = user_id);
drop trigger if exists trg_chart_calculations_updated on public.chart_calculations;
create trigger trg_chart_calculations_updated before update on public.chart_calculations
  for each row execute function public.set_updated_at();
...
create index if not exists idx_chart_versions_profile_computed on public.chart_json_versions(profile_id, computed_at desc);
create index if not exists idx_chart_versions_user on public.chart_json_versions(user_id);
...
create policy chart_json_versions_owner_select on public.chart_json_versions
  for select using (auth.uid() = user_id);
drop trigger if exists trg_chart_json_versions_immutable_update on public.chart_json_versions;
create trigger trg_chart_json_versions_immutable_update before update on public.chart_json_versions
...
create index if not exists idx_prediction_summaries_profile_topic on public.prediction_ready_summaries(profile_id, topic);
...
create policy prediction_ready_summaries_owner_select on public.prediction_ready_summaries
  for select using (auth.uid() = user_id);
drop trigger if exists trg_prediction_ready_summaries_immutable_update on public.prediction_ready_summaries;
create trigger trg_prediction_ready_summaries_immutable_update before update on public.prediction_ready_summaries
  for each row execute function public.prevent_immutable_update();
...
create index if not exists idx_astro_chat_sessions_user_created on public.astro_chat_sessions(user_id, created_at desc);
create index if not exists idx_astro_chat_sessions_profile on public.astro_chat_sessions(profile_id);
drop trigger if exists trg_astro_chat_sessions_updated on public.astro_chat_sessions;
create trigger trg_astro_chat_sessions_updated before update on public.astro_chat_sessions
  for each row execute function public.set_updated_at();
...
create policy astro_chat_sessions_owner_select on public.astro_chat_sessions
  for select using (auth.uid() = user_id);
...
create index if not exists idx_astro_chat_messages_session_created on public.astro_chat_messages(session_id, created_at);
create index if not exists idx_astro_chat_messages_user_created on public.astro_chat_messages(user_id, created_at desc);
...
create policy astro_chat_messages_owner_select on public.astro_chat_messages
  for select using (auth.uid() = user_id);
...
create index if not exists idx_calculation_audit_logs_calculation_created on public.calculation_audit_logs(calculation_id, created_at);
create index if not exists idx_calculation_audit_logs_user_created on public.calculation_audit_logs(user_id, created_at desc);
...
create policy calculation_audit_logs_owner_select on public.calculation_audit_logs
  for select using (auth.uid() = user_id);

create table if not exists public.astro_legacy_birth_chart_links (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  legacy_table text not null default 'birth_charts',
  legacy_id text not null,
  birth_profile_id uuid references public.birth_profiles(id) on delete set null,
  migration_status text not null default 'not_migrated'
    check (migration_status in ('not_migrated', 'migrated', 'verified', 'failed', 'ignored')),
  notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(legacy_table, legacy_id)
);
create index if not exists idx_astro_legacy_links_user on public.astro_legacy_birth_chart_links(user_id);
drop trigger if exists trg_astro_legacy_links_updated on public.astro_legacy_birth_chart_links;
create trigger trg_astro_legacy_links_updated before update on public.astro_legacy_birth_chart_links
  for each row execute function public.set_updated_at();
alter table public.astro_legacy_birth_chart_links enable row level security;
drop policy if exists astro_legacy_links_owner_select on public.astro_legacy_birth_chart_links;
create policy astro_legacy_links_owner_select on public.astro_legacy_birth_chart_links
  for select using (auth.uid() = user_id);
